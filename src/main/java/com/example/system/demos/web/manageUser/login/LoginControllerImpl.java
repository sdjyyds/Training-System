...
public class LoginControllerImpl implements LoginController {
    //注入
    @Autowired
    private LoginServixeImpl loginServixe;
    ...
    public void login(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        User user;
        // 1. 验证用户身份
        user = loginServixe.verifyIdentity(account, password);
...
//            response.sendRedirect("index/homeIndex.html");
            loginServixe.beforeLogin(user, request, response);
            //定向到下一个界面
            response.sendRedirect("index/homeIndex.html");
        } else {
...
//        }
        if (loginServixe.createHttpSession(login, token, request, session))
            return "{\"status\": \"success\", \"user\": \"" + login + "\", \"redirect\": \"../index/homeIndex.html\"}";
        return "{\"status\": \"fail\"}";
    }
